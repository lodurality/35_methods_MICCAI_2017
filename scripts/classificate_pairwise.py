...
class VectorFeaturizer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        vectors1 = data['pairs_data'].SUBID1.apply(lambda x: data['features'][x])
        vectors2 = data['pairs_data'].SUBID2.apply(lambda x: data['features'][x])
        features = []
...
if __name__ == "__main__":
    ...
    to_parse = sys.argv[1]
    dataset, tractography, model, con = to_parse.split('-')

    print('Dataset -- {},\nTractography -- {},\nModel -- {},\nCon --{}.'.format(
            dataset, tractography, model, con))

    overall_path = os.path.abspath('..')
    pairs_data_path = '{}/data/metadata/{}_phenotypic_data_pairwise.csv'.format(
                                                            overall_path, dataset)
    ...
    results = pipe.get_results(
        X=X,
        y=y,
        caching_steps= ['Normalizer', 'Featurizer', 'Pairwise_features'],
        scoring=['accuracy', 'roc_auc'])

    results_name = '_'.join([dataset, tractography, model, con]) + '.csv'
    folder = overall_path + '/results/script_results/pairwise_results/'
    results.insert(0, 'Dataset', results_name[:-4])

    if not os.path.exists(folder):
        os.makedirs(folder)

    results.to_csv(folder + results_name)
    print('Saved results to {}.'.format(folder + results_name))
    print("Done.")
